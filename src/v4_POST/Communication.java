...
class Communication {
    ...
    /**
     * This function is called once per round per bot. It gives each bot a
     * unique id between 0-49 that stays constant throughout the game, which
     * can be used for roles and other purposes.
     */
    public static void updateStandardIDs() throws GameActionException {
        RobotController rc = SharedVariables.rc;
        if (SharedVariables.standardID != -1) return;
        int previousStandardID = rc.readSharedArray(Constants.STANDARD_ID_IDX);
        SharedVariables.standardID = previousStandardID;
        rc.writeSharedArray(Constants.STANDARD_ID_IDX, previousStandardID + 1);
    }
