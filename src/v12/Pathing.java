...
public class Pathing {
    ...
    private static boolean tryMove(RobotController rc, Direction dir) throws GameActionException {
        ...
        else if ((!dir.equals(Direction.CENTER)) && (rc.canFill(rc.adjacentLocation(dir)) && rc.getCrumbs() > Constants.CRUMBS_TO_RESERVE_MEDIUM) && !isTooCloseToFlagCarrier(rc, rc.adjacentLocation(dir)))
        {
    ...
    private static Direction greedy(RobotController rc, MapLocation loc) throws GameActionException {
        ...
        for (int i = 0; i < Constants.directions.length; i++)
        {
            ...
            if ((rc.canMove(dir) || (rc.isMovementReady() && rc.canFill(next) && rc.getCrumbs() >= Constants.CRUMBS_TO_RESERVE_MEDIUM)) && !isTooCloseToFlagCarrier(rc, next))
            {
