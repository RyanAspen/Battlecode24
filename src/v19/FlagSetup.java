...
public class FlagSetup {
    ...
    private static boolean reachedBestLoc = false;
    ...
    private static int[] flagsMoved = new int[0]; //TODO: Needs to be in Comms
    ...
    public static void moveFlags(RobotController rc) throws GameActionException {
        ...
        {
            /*
            //Move the flag towards the non-held flag furthest from the dams
            int maxDistFromDams = 0;
            MapLocation bestLoc = null;
            for (int i = 0; i < SharedVariables.flagStatuses.length; i++)
            {
                if (!SharedVariables.flagStatuses[i].isHeldByAlly)
                {
                    int dist = Communication.getApproxDistToDams(rc, SharedVariables.flagStatuses[i].location);
                    if (dist > maxDistFromDams)
                    {
                        maxDistFromDams = dist;
                        bestLoc = SharedVariables.flagStatuses[i].location;
                    }
                }
            }
            ...
            if (bestLoc == null)
            {
                //If there is no bestStatus, move it towards the spawn that is furthest from the dams
                MapLocation[] allyLocations = rc.getAllySpawnLocations();
                for (int i = 0; i < allyLocations.length; i++)
                {
                    int dist = Communication.getApproxDistToDams(rc, allyLocations[i]);
                    if (dist > maxDistFromDams)
                    {
                        maxDistFromDams = dist;
                        bestLoc = allyLocations[i];
                    }
                }
            }

            //Move the flag close to bestLoc
            rc.setIndicatorString("Moving flag close to " + bestLoc);

            //If we get close to bestLoc, move to keep increasing damDistance
            if (bestLoc != null && bestLoc.isWithinDistanceSquared(rc.getLocation(), 4))
            {
                reachedBestLoc = true;
            }
            ...
            if (reachedBestLoc)
            {
                ...
                //Move to maximize minDamDistance, dropping the flag once possible
                int currDamDist = Communication.getApproxDistToDams(rc, rc.getLocation());
                int maxFlagDist = 0;
                ...
                for (int i = 0; i < Constants.directions.length; i++)
                {
                    ...
                    int damDist = Communication.getApproxDistToDams(rc, newLoc);
                    int flagDist = distToFlags(rc, newLoc);
                    if (damDist >= Constants.MIN_DIST_FROM_DAMS_FOR_FLAG)
                    {
                        if (flagDist > maxFlagDist)
                        {
                            maxFlagDist = flagDist;
                ...
                if (bestDir != null)
                    rc.move(bestDir);
                if (maxFlagDist >= 36 && rc.canDropFlag(rc.getLocation()))
                {
            ...
            {
                Pathing.moveTowards(rc, bestLoc);
            }

            Pathing.lingerTowards(rc, bestLoc, 45);
            */
