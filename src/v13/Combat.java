...
public class Combat {
    ...
    private static int getTrapCost(RobotController rc, TrapType type)
    {
    ...
    private static void damSetup(RobotController rc) throws GameActionException {
        if (rc.getRoundNum() > Constants.MIN_ROUND_RANGE_FROM_DAMS && rc.getRoundNum() < GameConstants.SETUP_ROUNDS)
        {
            ...
            if (farEnemies.length > 0)
            {
                ...
                if (rc.isActionReady() && Economy.canSpendAttack(rc, getTrapCost(rc, TrapType.STUN)) && distToDam < 3 && rc.canBuild(TrapType.STUN, rc.getLocation()))
                {
                    //Lay trap
                    Communication.logCrumbsAttack(rc, getTrapCost(rc, TrapType.STUN));
