...
public class Attacker {
    ...
    public static void runAttacker(RobotController rc) throws GameActionException {
        ...
        if (clustering)
        {
            System.out.print("I'm calling at: " + myCluster + ". I have " + (MAX_CLUSTER_TIME-clusterTimer) + " time left. There are " + allies.length + " allies.");
        }
        ...
        if (clustering)
        {
            if (allies.length >= MIN_CLUSTER_SIZE || clusterTimer >= MAX_CLUSTER_TIME)
            {
                ...
                System.out.println("Done with cluster " + myCluster);
                myCluster = null;
