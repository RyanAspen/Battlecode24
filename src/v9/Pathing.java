...
public class Pathing {
    ...
    static Direction moveTowards(RobotController rc, MapLocation target) throws GameActionException {
        ...
        if (rc.canMove(d) && !isTooCloseToFlagCarrier(rc, rc.getLocation().add(d))) {
            rc.move(d);
            movedIn = d;
            ...
            for (int i = 0; i < 8; i++) {
                if (rc.canMove(currentDirection) && !isTooCloseToFlagCarrier(rc, rc.getLocation().add(currentDirection))) {
    ...
    static Direction moveTowardsFlee(RobotController rc, MapLocation target, int minDist) throws GameActionException {
        ...
        if (rc.canMove(d) && (minDistToEnemyNew > minDist || minDistToEnemyNew > minDistToEnemy) && !isTooCloseToFlagCarrier(rc, rc.getLocation().add(d)))
        {
            rc.move(d);
            movedIn = d;
            currentDirection = null; // there is no obstacle we're going around
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (rc.canMove(currentDirection) && (minDistToEnemyNew > minDist || minDistToEnemyNew > minDistToEnemy) && !isTooCloseToFlagCarrier(rc, rc.getLocation().add(currentDirection))) {
                    rc.move(currentDirection);
                    movedIn = currentDirection;
                    currentDirection = currentDirection.rotateRight();
                    break;
