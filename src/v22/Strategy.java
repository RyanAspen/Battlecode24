...
public class Strategy {
    ...
    private static void changeRole(RobotController rc, Role role) throws GameActionException {
        ...
        {
            System.out.println("Becoming a " + role);
        ...
        else if (role != SharedVariables.currentRole)
        {
            ...
            {
                System.out.println("Becoming an Attacker from Defender");
                //Remove from comms
            ...
            {
                System.out.println("Becoming a Defender from Attacker");
                //Reset flagToDefend variables and increment counters once we get a new flag
                SharedVariables.flagIdToProtect = -1;
                SharedVariables.flagToProtect = null;
            }
    ...
    public static void changeStrategy(RobotController rc) throws GameActionException {
        ...
        int friendlyFlagCount = Communication.getFriendlyFlagsCount();
        ...
        if (SharedVariables.currentRole != null)
        {
            switch (SharedVariables.currentRole)
            {
                case Attacker:
                    ...
                    break;
                case Defender:
                    //Don't switch if we are the only ones defending a flag
                    FlagStatus currentStatus = Communication.getFlagStatus(SharedVariables.flagIdToProtect);
                    if (currentStatus != null && currentStatus.numberOfLiveDefenders <= Constants.MIN_DEFENDERS_PER_FLAG)
                    {
                        canSwitchRole = false;
                    }
                    else {
