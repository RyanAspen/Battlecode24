...
class BotProfile {
    ...
    void updateRank(int attackPower)
    {
        ...
        {
            rank = 8; //Experiment: Lowering priority of the flag holder
        }
        else if (closeToFlag)
        {
            rank = 3;
...
public class Combat {
    ...
    private static Order giveOrder(RobotController rc, boolean dontMove) throws GameActionException {
        ...
        //If a flag carrier is visible, never move to attack unless the target is a flag carrier
        boolean flagCarrierVisible = false;
        for (int i = 0; i < allEnemies.length; i++)
        {
            if (allEnemies[i].hasFlag())
            {
                flagCarrierVisible = true;
                break;
            }
        }
        ...
        for (int i = 0; i < profileEnemies.length; i++)
        {
            ...
            if (attackDir != null)
            {
                ...
                if (!attackDir.equals(Direction.CENTER) && (dontMove || (!flagCarrierVisible && profile.holdingFlag)))
                {
                    continue;
                }
    ...
    public static void micro(RobotController rc, boolean dontMove) throws GameActionException {
        damSetup(rc);
        if (rc.getRoundNum() >= GameConstants.SETUP_ROUNDS)
        {
            siegeCheck(rc);
            Order myOrder = giveOrder(rc, dontMove);
        ...
        RobotInfo[] closeEnemies = rc.senseNearbyRobots(-1, rc.getTeam().opponent());

        //If no enemies are seen, heal an ally
        if (closeEnemies.length == 0 && allies.length > 0 && rc.isActionReady())
        {
            int minHealth = 9999;
        ...
        if (!flagCarrierVisible && enemies.length > allies.length && rc.isMovementReady() && !dontMove)
        {
            evade(rc);
        }
        ...
        if (nearCrumbs.length > 0 && rc.isMovementReady() && !flagCarrierVisible && !dontMove)
        {
    ...
    public static boolean interceptFlagStealing(RobotController rc) throws GameActionException {
        ...
        {
            System.out.println("Flag to Defend = " + SharedVariables.flagToProtect);
        }
        ...
        for (int i = 0; i < enemies.length; i++)
        {
            ...
            if (SharedVariables.verbose)
            {
                System.out.println("PossibleStealLoc = " + possibleStealLoc);
            }
