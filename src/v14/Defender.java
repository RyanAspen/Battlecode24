...
public class Defender {
    ...
    public static void runDefender(RobotController rc) throws GameActionException {
        ...
        if (rc.getLocation().equals(flagToProtect))
        {
            ...
            if (rc.canBuild(TrapType.EXPLOSIVE, flagToProtect) && Economy.canSpendDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect))
            {
                Communication.logCrumbsDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect);
            ...
            else
            {
                ...
                if (enemies.length > 0)
                {
                    ...
                    if (closestDir != null && Economy.canSpendDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect) && rc.canBuild(TrapType.EXPLOSIVE, rc.getLocation().add(closestDir)))
                    {
                        Communication.logCrumbsDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect);
                        rc.build(TrapType.EXPLOSIVE, rc.getLocation().add(closestDir));
                    }
                }
                else if (Economy.canSpendDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect))
                {
                    for (int i = 0; i < Constants.directions.length; i++)
                    {
                        ...
                        if (rc.canBuild(TrapType.EXPLOSIVE, trapLoc) && Economy.canSpendDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect))
                        {
                            Communication.logCrumbsDefense(rc, Economy.getTrapCost(rc, TrapType.EXPLOSIVE), flagToProtect);
